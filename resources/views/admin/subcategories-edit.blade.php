        ...
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">
                    <i class="fa fa-fw fa-table"></i> Customer: {{$data['subcategory']->name}} </a>
                </h3>
            </div>
            <div class="panel-body">
                ...
                <div class="col-md-4">
                    <form class="form-horizontal" method="post" action="{{url('/subcategories/edit')}}">
                    {{ csrf_field() }}

                        <input type="hidden" name="id" value="{{$data['subcategory']->id}}" />
                        <input type="hidden" name="category_id" value="{{$data['subcategory']->category->id}}" />
                    <div class="form-group">
                        <label>Name</label>
                        <input type="text" name="name" value="{{$data['subcategory']->name}}" class="form-control" required="" />
                    </div>
                    <div class="form-group">
                        ...
                        <select style="width: 170px"  name="category_id" class="select2">
                            ...
                            ?>
                            <option value="<?php echo $cat->id; ?>" <?php if($data['subcategory']->category_id==$cat->id){ echo "selected";} ?>><?php echo $cat->name; ?></option>
                            <?php 
                            }
                            ?>
                    ...
                    <div class="form-group">
                        ...
                        <select name="deleted" class="form-control" >
                            <option <?php if($data['subcategory']->deleted==1){echo 'selected';} ?> value="1">Deleted</option>
                            <option <?php if($data['subcategory']->deleted==0){echo 'selected';} ?> value="0">Active</option>
                        </select>
                    </div>

                    <input type="submit" class="btn btn-sm btn-success" value="Update" />
                    <a href="{{url('/subcategories')}}" class="btn btn-sm btn-default">Cancel</a>
                </form>
                </div>
            </div>
        </div>
