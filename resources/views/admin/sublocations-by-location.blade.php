...
@section('title')
<u style="font-weight: bold;">Locations</u><i class="fa fa-arrow-right"></i><u style="font-weight: bold;">{{$data['location']['name']}}</u>
@endsection
        ...
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">
                    <i class="fa fa-fw fa-table"></i> Sub Locations | <a style="border-bottom: 1px dashed #FF0000;" href="" data-toggle="modal" data-target="#myModal"><i class="fa fa-plus-circle" ></i> Add New</a>
                </h3>
            </div>
                    ...
                    <table class="table table-striped table-bordered table-hover" id="sample_1">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo $sublocation['description']; ?></td>
                                <td>{{$sublocation->location->name}}</td>
                                <td>

                                    <form method="get" action="{{url('/sublocations/edit')}}">
                                        {{ csrf_field() }}
                                    ...
                                    <!-- Modal -->
                                    <div id="deleteSubLOcationConfirm_<?php echo $sublocation['id']; ?>" class="modal fade" role="dialog">
                                      <div class="modal-dialog">
                                        <!-- Modal content-->
                                        <div class="modal-content">
                                        <form class="form-horizontal" method="post" action="{{url('/delete-sublocation')}}">
                                          {{ csrf_field() }}
                                          ...
                                          <div class="modal-body">
                                              <div class="row" >
                                                <div class="col-md-12">
                                                    ...
                                                    <h4><?php echo $sublocation['name'] ?></h4>
                                                    <input type="hidden" name="id" value="<?php echo $sublocation['id']; ?>"/>
                                                    <input type="hidden" name="location_id" value="<?php echo $sublocation->location->id; ?>"/>
                                                </div>
                                              </div>
                                          </div>
                                ...
                                <td><button class="btn btn-sm btn-danger" data-toggle="modal" data-target="#deleteSubLOcationConfirm_<?php echo $sublocation['id']; ?>"><i class="glyphicon glyphicon-trash"></i>
                                    </button>
                                </td>
                                <td>
                                    <a href="{{url('locations/sublocations/childlocations')}}/{{$sublocation['id']}}" class="btn btn-success btn-sm">
                                        <i class="fa fa-list"></i> Child Locations
                                        (<b>{{count($sublocation->childlocations)}}</b>)
                                    </a>
                                </td>
                            </tr>
                            <?php
...
<div id="myModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <!-- Modal content-->
    <div class="modal-content">
        <form style="padding: 20px;" class="form-horizontal" method="post" action="{{url('/locations/sublocations')}}">
          {{ csrf_field() }}
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Add New Sublocation for "{{$data['location']['name']}}"</h4>
      </div>
      <div class="modal-body">
          <div class="row" >
            <div class="col-md-12">
